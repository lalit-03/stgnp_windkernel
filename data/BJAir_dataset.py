...
class BJAirDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.drop_attrs = ['PM25_Concentration', 'PM10_Concentration','NO2_Concentration', 'CO_Concentration', 'O3_Concentration', 'SO2_Concentration', 'wind_direction']
        ...
        self.test_node_index = self.get_node_division(test_nodes_path, num_nodes=self.raw_data['pred'].shape[0], seed = opt.seed)
    ...
    def load_loc(self, aq_location_path, build_adj=True):
        ...
        if build_adj:
            ...
            A = np.zeros((num_station, 2))
            for t in range(num_station):
                A[t, 0] = beijing_location.at[t, 'longitude']
                A[t, 1] = beijing_location.at[t, 'latitude']
    ...
    def load_feature(self, data_path, meta_path, time_division, delete_col=None):
        ...
        for id, station_aq in beijing_multimodal.groupby('station_id'):
            ...
            data['wind'].append(station_aq['wind_direction'].to_numpy()[np.newaxis])
        ...
        # print(data_length)
        # print(len(data['feat']), data['feat'][0].shape)
        # print(len(data['wind']), data['wind'][0].shape)
        # # exit()
        ...
        # data['wind'] = np.concatenate(data['wind'], axis=0)[:, start_index:end_index]
        data['wind'] = np.array([arr[:, start_index:end_index].reshape(-1, 1) for arr in data['wind']])
