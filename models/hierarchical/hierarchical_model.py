    ...
    def forward(self, training=True):
        self.p_y_pred, self.q_dists, self.p_dists, self.var_c, self.var_t = \
            self.netHierarchicalNP(self.feat_context, self.pred_context, self.feat_target,
                                   self.pred_target if training else None, self.missing_mask_context, training, self.A, self.context_index, self.target_index)
...
class HierarchicalNP(nn.Module):
    ...
    def forward(self, x_context, y_context, x_target, y_target, missing_index_context, training, A, context_index, target_index):
        ...
        adj = torch.vmap(process_data, in_dims=(0, 0, None))(target_index, context_index, A)

        # for storing lengthscale values
        # with open("lenscale_logs/lenscale_log_std_div_half.txt", "a") as f:
        #     f.write(str(self.lenscale.detach().cpu().numpy()))
        #     f.write('\n')
