...
from data.BJAir_dataset import BJAirDataset
...
class HierarchicalModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.A = input['A'][0].to(self.device)

        self.wind_mode = input['wind_mode'].to(self.device)

        self.context_index = input['context_index'].to(self.device)
        self.target_index = input['target_index'].to(self.device)

        # self.adj = torch.empty((0, 2, 3, 20))
        # self.adj.to(device = self.device, dtype = torch.float32)
    ...
    def forward(self, training=True):
        self.p_y_pred, self.q_dists, self.p_dists, self.var_c, self.var_t = \
            self.netHierarchicalNP(self.feat_context, self.pred_context, self.feat_target,
                                   self.pred_target if training else None, self.missing_mask_context, training, self.A, self.context_index, self.target_index, self.wind_mode, self.device)
...
class HierarchicalNP(nn.Module):
    def __init__(self, model_config):
        ...
        self.lenscale = nn.Parameter(torch.tensor(1.))

    def forward(self, x_context, y_context, x_target, y_target, missing_index_context, training, A, context_index, target_index, wind_mode, device):
        ...
        # wind_mode has shape (128, 35, 1). A has shape (35, 2), which is lat long of stations

        def get_wind_matrix(wind_direction):
            sin_squared = torch.sin(wind_direction)**2
            sin_cos = -torch.sin(wind_direction) * torch.cos(wind_direction)
            cos_squared = torch.cos(wind_direction) ** 2
            wind_matrix = torch.stack([sin_squared, sin_cos, sin_cos, cos_squared]).reshape(2, 2)
            return wind_matrix

        def get_wind_A(wind_directions, A):
            mode_direction, _ = torch.mode(wind_directions.flatten()) # take mode of wind directions
            wind_matrix = get_wind_matrix(mode_direction) # get wind matrix
            
            def compute_aij(xi, xj):
                tau = A[xi, 0:2] - A[xj, 0:2]
                tau = tau.reshape(2, 1).to(torch.float32)
                # x = torch.exp(-0.5 * torch.matmul(torch.matmul(tau.T, wind_matrix), tau))
                x = torch.exp(-0.5 * (tau.T @ wind_matrix @ tau))
                return x

            batch_size = A.shape[0]
            values = torch.stack([compute_aij(xi, xj) for xi in range(batch_size) for xj in range(batch_size)]).to(device)
            wind_A = values.view(batch_size, batch_size)
            return wind_A

        def process_iteration(targets, contexts, winds, A):
            A_ = get_wind_A(winds, A)
            A_ = torch.exp(torch.log(A_) / (self.lenscale)**2)
            A_1hop = (A_[targets, :][:, contexts]).unsqueeze(0)
            A_2hop = torch.mm(A_, A_)[targets, :][:, contexts].unsqueeze(0)
            temp = torch.cat((A_1hop, A_2hop), dim=0)
            temp = temp.float()
            return temp
        
        def process_batch(targets, contexts, winds):
            return process_iteration(targets, contexts, winds, A)

        adj = torch.vmap(process_batch, in_dims=(0, 0, 0))(target_index, context_index, wind_mode)
        # print(adj.shape)
